...
public sealed class IsapiClient : IDisposable
{
    ...
    public async Task<DeviceInfoModel> WaitForDeviceInfoAsync(TimeSpan timeout, TimeSpan retryInterval, CancellationToken cancellationToken)
    {
        ...
        throw new InvalidOperationException("Aktivasyon sonrasi 90 saniye boyunca /ISAPI/System/deviceInfo okunamadi.", lastException);
    }
    ...
    public async Task<IReadOnlyList<NetworkInterfaceModel>> UpdateGatewayDnsAsync(string? gatewayOverride, string dns1, string dns2, bool enableDhcp, CancellationToken cancellationToken)
    {
        var document = await GetXmlAsync("/ISAPI/System/Network/interfaces", cancellationToken).ConfigureAwait(false);
    ...
    public async Task<EnableEzvizResult> EnableEzvizAsync(string verificationCode, TimeSpan pollInterval, TimeSpan timeout, CancellationToken cancellationToken)
    {
        ...
        do
        {
            ...
            {
                return new EnableEzvizResult(latestStatus, pollCount, false);
            }
        ...
        latestStatus = await GetEzvizStatusAsync(cancellationToken).ConfigureAwait(false);
        return new EnableEzvizResult(latestStatus, pollCount, latestStatus.RegisterStatus != true);
    }
    ...
    private static Exception CreateRequestException(string method, string requestUri, HttpStatusCode statusCode, string body)
    {
        ...
        return new IsapiRequestException($"{method} {requestUri} basarisiz. HTTP {(int)statusCode} ({statusCode}). Yanit: {shortBody}", statusCode, body, TryExtractSubStatusCode(body));
    }
    ...
    private static NetworkInterfaceModel ParseNetworkInterface(XElement element)
    {
        ...
        return new NetworkInterfaceModel(EmptyToDash(id), EmptyToDash(ipAddress), EmptyToDash(subnetMask), EmptyToDash(gateway), EmptyToDash(primaryDns), EmptyToDash(secondaryDns), EmptyToDash(dhcpMode), element.ToString(SaveOptions.DisableFormatting));
    }
    ...
    private static string EmptyToDash(string value) => string.IsNullOrWhiteSpace(value) ? "-" : value.Trim();
    private static string NormalizeMac(string mac) => mac.Replace(":", "-", StringComparison.Ordinal).Trim().ToUpperInvariant();
