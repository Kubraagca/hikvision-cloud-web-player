...
function createTeamOpenApiService({
  ...
  async function getAreas() {
    const data = await postOpenApi("/api/hccgw/resource/v1/areas/get", {}, { operation: "areas.get" });
    const errorCode = String(data.errorCode || data.code || "");
    if (errorCode !== "0") {
  ...
  async function addDeviceAndImportChannels({ shortSerial, verificationCode, alias, areaId, areaName }) {
  ...
  async function addDeviceToAreaWorkflow({ shortSerial, verificationCode, alias, areaName }) {
    ...
    const area = await ensureArea(normalizedAreaName);
