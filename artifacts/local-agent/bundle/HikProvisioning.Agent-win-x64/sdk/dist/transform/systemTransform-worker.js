        ...
        if ("create" === data.type) {
            if (RECORDRTP) {
              ...
              postMessage({ type: "outputData", buf: data.buf, dType: 1 }, [data.buf]);
                ...
                var iRet = Module._CreatHandle(pHead, iTransType, iHeadLen);
                //-2147483645代表的是参数错误，此种情况，大概率发生在头信息错误，例如大华设备的情况，此时用 无头探测的策略
                //其他情况 按海康标准处理流程，不要做任何特殊处理
                if (iRet == -2147483645) {
                  iRet = Module._CreatHandle(0, iTransType, iHeadLen); //失败了，用探测的策略
                }
                if (iRet != 0) {
                    if(iRet == -2147483647) {
                  ...
                  if (data.options && typeof data.options.pKeyData !== "undefined") {
